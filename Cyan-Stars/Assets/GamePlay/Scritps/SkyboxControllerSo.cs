...
public class SkyboxControllerSo : MonoBehaviour
{
    ...
    public class RevolveKeyFrame
    {
        ...
        public float angle;
        [Header("缓动类型")]
        public SmoothFuncationType easeType;
    }
    ...
    void Update()
    {
        currentTime = GameMgr.Instance.TimelineTime * 1000;
        if(isStart)
        {
            if(revolveIndex < revolveKeyFrames.Count)
            {
                if(!onRevolve)
                {
                    ...
                    revolveKeyFrames[revolveIndex].time - revolveTimer, revolveKeyFrames[revolveIndex].easeType));
                    revolveIndex++;
                }
            }
            if(twinkleIndex < twinkleKeyFrames.Count)
            {
                ...
                {
                    StartCoroutine(TwinkleBreath(twinkleKeyFrames[twinkleIndex].minIntensity,twinkleKeyFrames[twinkleIndex].maxIntensity,twinkleKeyFrames[twinkleIndex].duration));
                    twinkleIndex++;
                }
            }
        }
    ...
    IEnumerator TwinkleBreath(float minIntensity, float maxIntensity, float duration)
    {
        ...
        {
            float intensity = (0.5f * Mathf.Cos(bpm * 2 * Mathf.PI * (currentTime - timePoint - 30000/bpm) / 60000) + 0.5f)
