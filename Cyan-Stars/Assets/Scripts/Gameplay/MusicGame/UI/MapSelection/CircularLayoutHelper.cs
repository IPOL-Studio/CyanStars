...
namespace CyanStars.Gameplay.MusicGame
{
    ...
    public static class CircularLayoutHelper
    {
        ...
        /// </summary>
        public static void SetItemXPos(RectTransform itemRect, RectTransform viewPortRect)
        {
            float itemW = itemRect.rect.width;
            float itemH = itemRect.rect.height;
            Rect vpRect = viewPortRect.rect;

            float yTop = vpRect.yMax + itemH;
            float yBottom = vpRect.yMin - itemH;
            float xLeft = vpRect.xMin - itemW;
            float xRight = vpRect.xMax + itemW;
