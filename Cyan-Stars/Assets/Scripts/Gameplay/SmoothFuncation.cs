...
namespace CyanStars.Gameplay
{
    public static class SmoothFuncation
    {
        ...
        public static Vector3
            SinFunctionEaseIn(Vector3 b, Vector3 e, float t, float dt) //正弦曲线的缓动（sin(t)）/ 从0开始加速的缓动，也就是先慢后快
        {
            return -(e - b) * Mathf.Cos(t / dt * (Mathf.PI / 2)) + (e - b) + b;
        }

        public static Vector3
            SinFunctionEaseOut(Vector3 b, Vector3 e, float t, float dt) //正弦曲线的缓动（sin(t)）/ 减速到0的缓动，也就是先快后慢
        {
            return (e - b) * Mathf.Sin(t / dt * (Mathf.PI / 2)) + b;
        }

        public static Vector3
            SinFunctionEaseInOut(Vector3 b, Vector3 e, float t, float dt) //正弦曲线的缓动（sin(t)）/ 前半段从0开始加速，后半段减速到0的缓动
        {
            return -(e - b) / 2 * (Mathf.Cos(Mathf.PI * t / dt) - 1) + b;
        }

        public static Vector3
            BackEaseIn(Vector3 b, Vector3 e, float t, float dt) //超过范围的三次方缓动（(s+1)*t^3 – s*t^2）/ 从0开始加速的缓动，也就是先慢后快
        {
            float s = 1.70158f;
        ...
        public static float
            SinFunctionEaseInOut(float b, float e, float t, float dt) //正弦曲线的缓动（sin(t)）/ 前半段从0开始加速，后半段减速到0的缓动
        {
            return -(e - b) / 2 * (Mathf.Cos(Mathf.PI * t / dt) - 1) + b;
        }

        public static float
            BackEaseIn(float b, float e, float t, float dt) //超过范围的三次方缓动（(s+1)*t^3 – s*t^2）/ 从0开始加速的缓动，也就是先慢后快
        {
            float s = 1.70158f;
            return (e - b) * (t /= dt) * t * ((s + 1) * t - s) + b;
        }
